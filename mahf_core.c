/*
 * Mahf-Lambea4 CPU Platform - Kernel Mode Driver
...
)
{
    ...
    DbgPrint("Mahf-Lambea4 CPU Platform %d.%d.%d Loading...\n",
    ...
    DbgPrint("Mahf-Lambea4 CPU Platform loaded successfully\n");
...
)
{
    ...
    // Create device name
    RtlInitUnicodeString(&deviceName, L"\\Device\\MahfLambea4CPU");
    ...
    RtlInitUnicodeString(&symbolicLink, L"\\DosDevices\\MahfLambea4CPU");
    ...
    status = WdfDeviceCreateDeviceInterface(device,
                                            &GUID_DEVINTERFACE_MAHF_LAMBEA4_CPU,
                                            NULL);
    if (!NT_SUCCESS(status)) {
...
NTSTATUS HandleIOCTL(PDRIVER_CONTEXT Context, WDFREQUEST Request, ULONG IoControlCode)
{
    ...
    switch (IoControlCode) {
        case IOCTL_MAHF_LAMBEA4_GET_CPU_INFO:
            status = GetCPUInfo(Context, outputBuffer, outputLength);
            break;

        case IOCTL_MAHF_LAMBEA4_GET_PERFORMANCE_DATA:
            status = GetPerformanceData(Context, outputBuffer, outputLength);
            break;

        case IOCTL_MAHF_LAMBEA4_SET_PERFORMANCE_STATE:
        ...
        case IOCTL_MAHF_LAMBEA4_RESET_DRIVER:
            status = InitializeDriverContext(Context);
            break;
...
VOID OnDriverUnload(WDFDRIVER Driver)
{
    ...
    DbgPrint("Mahf-Lambea4 CPU Platform %d.%d.%d Unloading\n",
             DRIVER_VERSION_MAJOR, DRIVER_VERSION_MINOR, DRIVER_VERSION_BUILD);
}
